...
doner = lambda x :  colored(x , 'green')
failr = lambda x :  colored(x , 'red') 
warnr = lambda x :  colored(x , 'yellow' )
...
if __name__ == '__main__':
	...
	parser.add_argument('--id', default = None ,\
		help = 'id of the device we address the command. --id <ip>:<port> '+\
	...
	if ip is None or port is None:
			if command in ['depart' , 'overlay', 'greet']:
				...
				ip = ips[randint(0,6)%5]
				port = ports[randint(0,3)%2]

	BASE_URL = 'http://' + str(ip) + ':' + str(port) + PREFIX

	if command == 'greet':
		res = get_greet(ip, port)
		print(doner('current node:') , res['current'])
		print(doner('next node: '), res['next'])
		print(doner('previous node:'), res['prev'])
		print(doner('replication factor:'), res['replication'])
		print(doner('policy: '), res['policy'])
		print(doner('primary keys:'), res['resps'])
		print(doner('replicas: '), res['replicas'])
		...
		if args.filename is not None:
			...
			with open(args.filename , 'r') as f:
				for line in f:
					...
					if res['status'] == 200 or res['status'] == '200':
						print(doner('found key') ,res['key'], 'with value', res['value'])
				end = time.time()
				dur = end-start
				print('Elapsed time: {} min : {:.2f} secs'.format(dur // 60, dur%60))
			exit(0)
			...
			if key == '*':
				...
				for k,v in val.items():
					txt = colored('node : {}'.format(k), 'green')
					print(txt + ' | (key,value) pairs: {}'.format(v))
			else:
				print(res)
		...
		with open(args.filename, 'r') as f:
			...
			for line in f:
				...
				if line[0].strip() == 'insert':
					...
					print(doner('inserted'), '{} key with value {}'.format(key, value))
					#print(res)
				elif line[0].strip() == 'query':
					key = line[1].strip().strip('\n')
					res = query_function(ip, port, key)
					print(doner('found') ,'result for key : {} is value: {}'.format(res['key'],res['value']))
					#print(res)
				else:
					print('error, exiting...')
					exit(1)
